...
class Machine:
    def __init__(self) -> None:
        ...
        # import sounds
        self.spin_sound = pygame.mixer.Sound('audio/spinclip.wav')
    ...
    def coolsdowns(self):
        ...
        if not self.can_toggle and [self.reel_list[reel].reel_is_spinning for reel in self.reel_list].count(False) == 5:
            ...
            if self.check_wins(self.spin_result):
                ...
                # print(self.currPlayer.get_data(), ' - win')
    ...
    def check_wins(self, result):
        ...
        for row in horizontal:
            for sym in row:
                if row.count(sym) > 2:  # Potential result
                    possible_win = [idx for idx, val in enumerate(row) if sym == val]

                    # Check possible_win for a subsequqnce longer than 2 and add to hits
